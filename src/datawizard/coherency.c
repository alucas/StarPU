				...
				void (*callback_func)(void *), void *callback_arg)
{
	...
	if (!r) {
		...
		if ((mode & STARPU_R) && (src_is_a_gpu && dst_is_a_gpu)) {
			...
			r_ram_to_dst = _starpu_create_data_request(handle, 0, handle, requesting_node, requesting_node, mode, is_prefetch);
			...
			if (!r_src_to_ram)
			{
				...
				r_src_to_ram = _starpu_create_data_request(handle, src_node, handle, 0, src_node, mode, is_prefetch);
			}
			else {
			...
			r = _starpu_create_data_request(handle, src_node, handle, requesting_node, handling_node, mode, is_prefetch);
